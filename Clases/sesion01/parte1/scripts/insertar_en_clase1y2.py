...
with open('../datos/slides_clase1y2.json', 'r', encoding='utf-8') as f:
    data = json.load(f)

slides = data['slides']

# Buscar el slide "Cinco etapas del comercio internacional" (slide 21)
insert_after = None
for i, s in enumerate(slides):
    if 'Cinco etapas' in s.get('titulo', ''):
        insert_after = i
        print(f"Encontrado: slide {s['numero']} '{s['titulo']}' en index {i}")
        break

if insert_after is None:
    print("ERROR: No encontré el slide 'Cinco etapas'")
    sys.exit(1)

# Verificar que no estén ya insertados
for s in slides:
    if 'Convergencia' in s.get('titulo', '') or 'Gran Divergencia' in s.get('titulo', ''):
        print(f"AVISO: Ya existe slide '{s['titulo']}'. No inserto de nuevo.")
        sys.exit(0)
...
# Insertar después del slide encontrado
for i, s in enumerate(nuevos):
    slides.insert(insert_after + 1 + i, s)
...
with open('../datos/slides_clase1y2.json', 'w', encoding='utf-8') as f:
    json.dump(data, f, ensure_ascii=False, indent=2)

print(f"\nJSON actualizado: {len(slides)} slides")
print("\nListado completo:")
for s in slides:
    v = s.get('visual') or ''
    if v:
        v = f'  [{v}]'
    print(f"  {s['numero']:3d}. ({s['tipo']}) {s['titulo']}{v}")
