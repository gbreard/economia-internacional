...
def render_centrado(s, active):
    items = '\n'.join(f'        <li style="margin-bottom: 12px;">{esc(c)}</li>' for c in s.get('contenido', []))
    return f'''
  <section class="slide{active}" style="
    background-color: #FFFFFF;
    display: flex; flex-direction: column; justify-content: center; align-items: center;
...
def render_agenda(s, active):
    items = '\n'.join(f'        <li style="margin-bottom: 15px;">{esc(c)}</li>' for c in s.get('contenido', []))
    return f'''
  <section class="slide{active}" style="
    background-color: #FFFFFF;
    display: flex; flex-direction: column;
...
def render_grafico_texto(s, active):
    ...
    if visual and not visual.startswith('../'):
        visual = '../' + visual
    fuente = s.get('fuente', '')
    fuente_html = f'<div style="font-size: 0.75rem; color: #4B5563; font-style: italic; text-align: center; margin-top: 8px;">Fuente: {esc(fuente)}</div>' if fuente else ''
    items = '\n'.join(f'          <li style="margin-bottom: 8px;">{esc(c)}</li>' for c in s.get('contenido', []))
    return f'''
  <section class="slide{active}" style="
    background-color: #FFFFFF;
    display: flex; flex-direction: column;
...
<head>
  ...
  <title>Economía Internacional - Clase 3</title>
  <style>
...
<div class="footer">
  <span>Economía Internacional | UMET | Clase 3</span>
